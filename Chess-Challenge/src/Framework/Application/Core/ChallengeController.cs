...
namespace ChessChallenge.Application
{
    public class ChallengeController
    {
        ...
        public ChallengeController()
        {
            ...
            StartNewGame(PlayerType.Human, PlayerType.MyBotLatest);
        }
        ...
        public void StartNewUnlimitedGame(PlayerType whiteType, PlayerType blackType)
        {
            // End any ongoing game
            EndGame(GameResult.DrawByArbiter, log: false, autoStartNextBotMatch: false);
            gameID = rng.Next();
            ...
            PlayerBlack = CreatePlayer(blackType, true);
            PlayerWhite.SubscribeToMoveChosenEventIfHuman(OnMoveChosen);
            PlayerBlack.SubscribeToMoveChosenEventIfHuman(OnMoveChosen);
        ...
        ChessPlayer CreatePlayer(PlayerType type, bool unlimited = false)
        {
            return type switch
            {
                PlayerType.MyBotLatest => VersionHelper.getLatest(type, unlimited ? int.MaxValue : GameDurationMilliseconds),
                PlayerType.MyBotPrevious => VersionHelper.getPrevious(type, unlimited ? int.MaxValue : GameDurationMilliseconds),
                PlayerType.EvilBot => new ChessPlayer(new EvilBot(), type, unlimited ? int.MaxValue : GameDurationMilliseconds),
        ...
        public void Update()
        {
            ...
            if (isPlaying)
            {
                if (Unlimited && PlayerToMove.IsBot)
                {
                    int keypress = -1;
                    while (keypress != 0)
                    {
                        keypress = Raylib.GetKeyPressed();
                        if (keypress == (int)KeyboardKey.KEY_ENTER) PlayerToMove.Bot.Stop = true;
                    }
                }
                ...
                if (PlayerToMove.TimeRemainingMs <= 0 && !Unlimited)
                {
        ...
        public void StartNewUnlimitedBotMatch(PlayerType botTypeA, PlayerType botTypeB)
        {
            Unlimited = true;
            ...
            botAPlaysWhite = true;
            Log($"Starting new match: {nameA} vs {nameB}", false, ConsoleColor.Blue);
            StartNewUnlimitedGame(botTypeA, botTypeB);
        }
