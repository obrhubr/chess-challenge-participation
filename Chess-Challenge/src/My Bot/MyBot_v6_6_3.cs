...
// Add Quiescence Search only if the last move was a capture
public class MyBot_v6_6_3 : IChessBot
{
    ...
    private static readonly int quiescenceDepth = 10;
    // Transposition table
    ...
    private (double, Move) Minimax(ChessChallenge.API.Timer timer, Board board, int depth, int maxDepth, double alpha, double beta, Move lastBestMove, int numExtensions, bool doQuiescence)
    {
        ...
        if (movesUnordered.Length == 0 || depth == 0)
        {
            if (doQuiescence) return (QuiescenceSearch(timer, board, alpha, beta), new Move());
            else return ((board.IsWhiteToMove ? 1 : -1) * Evaluate(board), new Move());
        }
        ...
        foreach (Move move in moves)
        {
            ...
            // If the move is later in the move list search less deeply
                ...
                searchDepth--;
            var (moveEvaluation, _) = Minimax(timer, board, searchDepth, maxDepth, -beta, -value, new Move(), numExtensions, move.IsCapture);

            // If the move seems interesting in spite of reducing the search depth, search in full
            if (searchDepth < (depth - 1) && (-1 * moveEvaluation) > alpha) (moveEvaluation, _) = Minimax(timer, board, depth - 1, maxDepth, -beta, -value, new Move(), numExtensions, move.IsCapture);
    ...
    private Move IterativeDeepening(ChessChallenge.API.Timer timer, Board board)
    {
        ...
        while (timer.MillisecondsElapsedThisTurn < (thinkingTime + 2))
        {
            ...
            var (value, move) = Minimax(timer, board, depth, depth, -Max, Max, bestMove, 16, false);
