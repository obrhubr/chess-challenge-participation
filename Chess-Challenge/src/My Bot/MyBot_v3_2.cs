...
public class MyBot_v3_2 : IChessBot
{
    ...
    private static (Double, Move) Minimax(Board board, int depth, bool maximisingPlayer, double alpha, double beta)
    {
        ...
        {
            //if (repetitionTable.ContainsKey(board.ZobristKey)) return (maximisingPlayer ? Max : -Max, new Move());
            if (board.IsRepeatedPosition()) return (maximisingPlayer ? Max : -Max, new Move());
            if (board.FiftyMoveCounter >= 100) return (maximisingPlayer ? Max : -Max, new Move());
        }
    ...
    {
        (Double, Move) m = Minimax(board, maxDepth, board.IsWhiteToMove, -Max, Max);
        return m.Item2;
    }
}