...
// Search last moves less deeply
public class MyBot_v6_4_0 : IChessBot
{
    ...
    private (double, Move) Minimax(ChessChallenge.API.Timer timer, Board board, int depth, int maxDepth, double alpha, double beta, Move lastBestMove)
    {
        ...
        foreach (Move move in moves)
        {
            ...
            board.MakeMove(move);
            if (!move.IsCapture && depth >= 3 && i >= 3) searchDepth--;
            var (moveEvaluation, _) = Minimax(timer, board, searchDepth, maxDepth, -beta, -value, new Move());
            // If the move seems interesting in spite of reducing the search depth, search in full
            if (searchDepth < (depth - 1) && (-1*moveEvaluation) > alpha) (moveEvaluation, _) = Minimax(timer, board, depth - 1, maxDepth, -beta, -value, new Move());
            board.UndoMove(move);
