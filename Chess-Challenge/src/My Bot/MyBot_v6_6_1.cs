...
// Add statistics
public class MyBot_v6_6_1 : IChessBot
{
    ...
    public struct Stats
    {
        ...
        public int quiescence { get; set; }
        public int extensions { get; set; }
        public int lessdeeply { get; set; }
        public int lessdeeplyfailed { get; set; }
    }
    ...
    private (double, List<Move>) Minimax(ChessChallenge.API.Timer timer, Board board, int depth, int maxDepth, double alpha, double beta, Move lastBestMove, int numExtensions)
    {
        // Update statistics
        ...
        if (movesUnordered.Length == 0 || depth == 0) return (QuiescenceSearch(timer, board, alpha, beta), new List<Move>());
        ...
        foreach (Move move in moves)
        {
            ...
            var (moveEvaluation, searchedMoves) = Minimax(timer, board, searchDepth, maxDepth, -beta, -value, new Move(), numExtensions);

            // If the move seems interesting in spite of reducing the search depth, search in full
            if (searchDepth < (depth - 1) && (-1 * moveEvaluation) > alpha)
            {
                stats.lessdeeplyfailed++;
                (moveEvaluation, searchedMoves) = Minimax(timer, board, depth - 1, maxDepth, -beta, -value, new Move(), numExtensions);
            }
    ...
    private Move IterativeDeepening(ChessChallenge.API.Timer timer, Board board)
    {
        ...
        while ((timer.MillisecondsElapsedThisTurn < (thinkingTime + 2) && !unlimited) || (!_stop && unlimited))
        {
            ...
            var (value, moves) = Minimax(timer, board, depth, depth, -Max, Max, bestMove, 16);
