...
// Do not throw away the best move from a not completed iterative deepening round by adding move ordering with EvaluateMove
public class MyBot_v4_1 : IChessBot
{
    ...
    private static (Double, Move) Minimax(Timer timer, Board board, int depth, int maxDepth, double alpha, double beta, Move lastBestMove)
    {
        ...
        Move[] moves;
        if (lastBestMove.TargetSquare != lastBestMove.StartSquare) moves = movesUnordered.OrderByDescending(m => EvaluateMove(board, m)).ToList().Prepend(lastBestMove).ToArray();
